...
- git_lfs:
  ...
  Logical, whether to automatically opt into Git LFS to track large
  files in `_targets/objects` more efficiently. If `TRUE` and Git LFS is
  installed, it should work automatically. If `FALSE`, you can always
  opt in later by running `git lfs track objects` inside the data store.
...
`tar_git_init()` also writes a `.gitattributes` file to the store to
automatically track target output date with `git-lfs` if it is installed
on your system.
